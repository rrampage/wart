...
public class WasmParserTest {
    private static final WasmParserTestCase[] testCases = new WasmParserTestCase[] {
            new WasmParserTestCase("empty_module.wasm", 8, 0, 0,0, 0, 0),
            new WasmParserTestCase("add_two.wasm", 82, 3, 1, 3, 1, 0),
            new WasmParserTestCase("fizzbuzz_manual.wasm", 326, 4, 1, 6, 0, 2),
            new WasmParserTestCase("import_global.wasm", 104, 2, 5, 0, 0, 0),
            new WasmParserTestCase("walloc.wasm", 2023, 3, 1, 6, 2, 2),
            new WasmParserTestCase("rocket.wasm", 53067, 17, 9, 125, 8, 198),
            new WasmParserTestCase("data_segment_example.wasm", 88, 1, 1, 1, 0, 2),
    };

    record WasmParserTestCase(String fileName, int numBytes, int numTypes, int numImports, int numFunctions, int numExports, int numDataSegments) {
        public void check() {
